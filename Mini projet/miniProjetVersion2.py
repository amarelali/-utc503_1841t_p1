...
def moyenneEtudiant(etudiant , dbNote):
    ...
    data =[]
    for note in dbNote: 
        idEtud.append(note.numeroEtudiant)   
    
    filterId= list(filter(lambda  x : x == etudiant.numeroEtudiant , idEtud))
    for note in dbNote: 
        if note.numeroEtudiant == etudiant.numeroEtudiant :
            data.append(note.note) 
    addition = lambda x,y:x + y            
    
    return functools.reduce(addition,data)/len(filterId)
